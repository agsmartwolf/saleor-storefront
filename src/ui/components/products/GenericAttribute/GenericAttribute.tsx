					...
					{attribute.values.map((v) => (
						<RadioGroup.Option
							...
							className={({ checked }) =>
								clsx({
									"bg-brand": checked,
									"bg-white": !checked,
									"strikethrough-diagonal border-gray-200 text-gray-200": !product?.variants
										?.filter(
											(_var) =>
						...
						>
							{({ checked }) => (
								<div
									className={clsx(
										"relative inline-flex cursor-pointer border-[1px] object-contain",
										checked && "border-brand",
										!checked && "border-main-2 hover:border-main",
									)}
								>
									<RadioGroup.Label
										as="div"
										className="inline-flex whitespace-nowrap px-4 py-1 text-sm font-semibold"
	...
	const colors = useMemo(() => {
		return attribute.values
			...
			.map((v) => ({
				...
				active: Boolean(
					searchParams.get(attribute.attribute.id) === v.id ||
