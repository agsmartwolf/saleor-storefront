...
import { type Attribute, type AttributeValue, type Product, ProductVariant } from "@/gql/graphql";
...
export const getSelectedVariant = ({
  ...
  const isAllRequiredAttributesInURL = attrsKeys.every((k) => !!(
  typeof searchParams?.get !== 'undefined' ? (searchParams as ReadonlyURLSearchParams).get(k)
  ...
  const curVar = product?.variants?.find((v) =>
    v.attributes
      ...
      .every((a) => {
        const curAttrQueryVal = typeof searchParams?.get !== 'undefined' ? (searchParams as ReadonlyURLSearchParams).get(a.attribute.id)
